...
This module contains quite a lot of *magic*, so beware.
"""
...
class NetrefMetaclass(type):
    ...
    def __repr__(cls):
        if cls.__module__:
            return f'<netref class {cls.__module__}.{cls.__name__}'
        else:
            return f'<netref class {cls.__name__}>'


class BaseNetref(metaclass=NetrefMetaclass):
    """The base netref class, from which all netref classes derive.

    Some netref classes are "pre-generated" and cached upon importing this
    module (those defined in the :data:`_builtin_types`), and they are shared
    between all connections.

    The rest of the netref classes are created by
    :meth:`rpyc.core.protocl.Connection._unbox`, and are private to the
    connection.

    Do not use this class directly; use :func:`class_factory` instead.

    :param conn:
        The :class:`rpyc.core.protocol.Connection` instance
    :param id_pack:
        ID tuple for an object. A tuple of (name_pack, remote-class-id,
        remote-instance-id).

        name_pack := __module__.__name__ (hits or misses on builtin cache and sys.module)
        ...
        The id_pack is usually created by rpyc.lib.get_id_pack.
    """
    ...
    def __del__(self):
        try:
            ...
            # Raised in a destructor, most likely on program termination, when
            # the connection might have already been closed. It's safe to
